...
# This silently checks for and installs the newest version of yfinance when the app wakes up
try:
    subprocess.check_call([sys.executable, "-m", "pip",
                          "install", "--upgrade", "--quiet", "yfinance"])
except Exception:
    pass  # If it fails, just ignore it and run the app anyway
...
# --- 2. ROBUST P/E EXTRACTION & CALCULATION ---
        pe = info.get('trailingPE')

        # If Yahoo's trailingPE is blank or None, check forwardPE
        if pe is None or str(pe).strip() == "" or str(pe).lower() == "nan":
            pe = info.get('forwardPE')

        # If both are missing, calculate it manually using Live Price / EPS
        if pe is None or str(pe).strip() == "" or str(pe).lower() == "nan":
            live_price = info.get('currentPrice', info.get('previousClose'))
            eps = info.get('trailingEps')
            # Only calculate if EPS is positive (company is profitable)
        ...
        # THE CATCH-ALL FIX: If P/E is negative (Net Loss), force it to NaN (N/A)
        if pd.notna(pe) and pe <= 0:
            pe = np.nan

        # --- 3. Robust Sector Extraction ---
...
with tab1:
    if not df.empty:
        ...
        # Metric Cards with CAD/USD Book Value
