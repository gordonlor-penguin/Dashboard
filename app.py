...
# --- 3. HYBRID ETF SCRAPING ENGINE (INVESCO & VANGUARD OPTIMIZED) ---
...
# --- NEW: TRENDING REDDIT STOCKS ENGINE ---


@st.cache_data(ttl=1800)  # Updates every 30 minutes
def get_trending_reddit_stocks():
    try:
        url = "https://tradestie.com/api/v1/apps/reddit"
        res = requests.get(url, timeout=5)
        if res.status_code == 200:
            data = res.json()
            return data[:10]  # Return the top 10 most talked about
    except:
        pass
    return []
...
if page == "📈 Command Center":
    if not df.empty:
        ...
        # --- REPLACED: NEW REDDIT TRENDING EXPANDER ---
        with st.expander("🔥 Trending on Reddit (WallStreetBets)", expanded=False):
            with st.spinner("Scanning latest posts on r/WallStreetBets..."):
                reddit_trends = get_trending_reddit_stocks()
                if reddit_trends:
                    st.markdown(
                        "These are the **Top 10 most discussed stocks & ETFs** on Reddit right now:")

                    trend_df = pd.DataFrame(reddit_trends)
                    trend_df = trend_df.rename(columns={
                        "ticker": "Ticker",
                        "sentiment": "Sentiment",
                        "no_of_comments": "Mentions in the last hour",
                        "sentiment_score": "Score"
                    })

                    # Function to color code the sentiment Bullish/Bearish
                    def style_sentiment(val):
                        color = '#00C9B1' if val == 'Bullish' else '#FF4B4B'
                        return f'color: {color}; font-weight: bold;'

                    st.dataframe(
                        trend_df[["Ticker", "Mentions in the last hour", "Sentiment"]].style.applymap(
                            style_sentiment, subset=["Sentiment"]),
                        use_container_width=True, hide_index=True
                    )
                else:
                    st.info(
                        "Reddit trending data is currently unavailable. Try again later.")
