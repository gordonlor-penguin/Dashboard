...
if page == "📈 Command Center":
    if not df.empty:
        ...
        # COMMA SEPARATORS ADDED FOR ALL NUMERIC FIELDS
        st.dataframe(
        ...
        # GEOGRAPHIC MAP EXPANDER (WITH ETF LOOK-THROUGH)
        with st.expander("🌍 Geographic Exposure Map (True Underlying Exposure)", expanded=False):
            with st.spinner("Calculating True ETF Look-Through..."):
                ...
                for _, row in grouped[grouped["Value (CAD)"] > 0].iterrows():
                    val = row["Value (CAD)"]
                    ticker = row["Ticker Symbol"]
                    base_country = row["Country"]
                    sector = str(row["Sector"])

                    # If it's an ETF and we have the FMP API key, look inside it!
                    if "ETF" in sector and fmp_key:
                        try:
                            # FMP supports .TO for TSX
                            fmp_url = f"https://financialmodelingprep.com/api/v3/etf-country-weightings/{ticker}?apikey={fmp_key}"
                            res = requests.get(fmp_url, timeout=3).json()
                            ...
                            if res and isinstance(res, list) and len(res) > 0:
                                ...
                                for item in res:
                                    country_name = item.get(
                                        'country', 'Unknown')
                                    # Handle FMP's string percentages (e.g. "98.54%")
                                    weight_str = str(
                                        item.get('weightPercentage', '0')).replace('%', '')
                                    ...
                                    exploded_geo_rows.append(
                                        {"Country": country_name, "Value (CAD)": allocated_val})

                                # Catch any unallocated remainder
                                if total_allocated < 1.0:
                                    exploded_geo_rows.append(
                                        {"Country": base_country, "Value (CAD)": val * (1.0 - total_allocated)})
                                continue  # Skip appending the base country since we exploded it
                        except:
                            pass  # Fallback to base country if API fails

                    # If it's a single stock (or ETF look-through failed), assign 100% to its home country
                    exploded_geo_rows.append(
                ...
                fig_map = px.choropleth(
                    ...
                    title="True Geographic Exposure (ETF Look-Through Applied)"
                )
                ...
                # Show exactly how much money is in each country
                st.dataframe(
    ...
    if not df.empty and total_val_cad > 0:
        ...
        # COMMA SEPARATORS FOR REBALANCE LEDGER
        st.dataframe(
