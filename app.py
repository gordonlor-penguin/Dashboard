...
def load_data():
    ...
    # Handle files that just say "MER"
    ...
    # NEW: Strips string percentages if you type "0.03%" into the CSV manually
...
def get_smart_metadata(ticker_str):
    ...
    # Pre-emptively assign country to kill the "Unknown" issue entirely
    ...
    try:
        ...
        # Note: MER logic has been completely ripped out so your manual inputs are safely preserved
...
def get_market_data(tickers_tuple):
    hist_prices, prices = {}, {}
    try:
        fx_hist = yf.Ticker("USDCAD=X", session=yf_session).history(
            period="5y")['Close']
    ...
    def fetch_single_ticker(t):
        ...
        try:
            t_hist = yf.Ticker(t, session=yf_session).history(
                period="5y")['Close']
            if not t_hist.empty:
...
def get_benchmarks(tickers_tuple):
    ...
    def fetch_bench(t):
        ...
        try:
            df_b = yf.Ticker(t, session=yf_session).history(
                period="5y")['Close']
            if not df_b.empty:
...
with st.sidebar:
    ...
    # --- NEW: REFRESH BUTTON TO FIX "UNKNOWN" DATA CACHES ---
...
if not df.empty:
    ...
    with st.spinner("Syncing Live Engines (Accelerated)..."):
        ...
        for idx, row in df.iterrows():
            ...
            needs_update = False
            # We ONLY look for missing Sector or Country now!
                ...
                if len(opt_tickers) > 1:
                    ...
                    if len(valid_hist_data) > 1:
                        ...
                        with c2:
                            ...
                            st.plotly_chart(fig_opt.update_layout(barmode='group', template="plotly_dark",
                                            title="Your Current vs AI Optimal Allocations", xaxis_title="", yaxis_title="Weight (%)"), use_container_width=True)
                    else:
                        st.error(
