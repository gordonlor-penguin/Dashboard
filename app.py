...
if page == "📈 Command Center":
    if not df.empty:
        ...
        # --- TRUE RECURSIVE GEOGRAPHIC MAP & TOP 10 HOLDINGS WITH EXACT BREAKDOWNS ---
        def get_etf_remainder_breakdown(ticker, base):
            """Returns a dictionary to break down unaccounted percentages into specific countries."""
            t = ticker.upper()
            ...
            if t in ["VIU.TO", "VEA", "XEF.TO"]:
                return {"Japan": 0.22, "United Kingdom": 0.13, "France": 0.10, "Switzerland": 0.09, "Germany": 0.08, "Australia": 0.07, "Netherlands": 0.05, "Other Developed": 0.26}
            if t in ["VEE.TO", "VWO", "XEC.TO"]:
                return {"China": 0.26, "India": 0.19, "Taiwan": 0.18, "Brazil": 0.05, "Saudi Arabia": 0.04, "South Africa": 0.03, "Other Emerging": 0.25}
            return {base: 1.0}
        ...
        with st.expander("🌍 True Geographic & Top 10 Holdings Exposure", expanded=False):
            with st.spinner("Executing Recursive Look-Through of Nested ETFs..."):
                ...
                # Extended list of known ETFs to trigger multiple recursive loops
                KNOWN_RECURSIVE_ETFS = ["VTI", "VFV.TO", "VEQT.TO", "VUN.TO", "VCN.TO", "VIU.TO", "VEE.TO", "VOO",
                                        "QQQ", "QQC", "QQC.TO", "XEQT.TO", "ITOT", "IVV", "SPY", "VEA", "VWO", "XIC.TO", "XEF.TO", "XEC.TO"]
                ...
                for _, row in grouped[grouped["FMV (CAD)"] > 0].iterrows():
                    ...
                    if "ETF" in sector:
                        # Deep-Dive Breadth First Search Loop
                            ...
                            if holdings_df is not None and not holdings_df.empty:
                                ...
                                for _, h_row in holdings_df.iterrows():
                                    ...
                                    if weight > 0:
                                        ...
                                        # If an ETF holds an ETF, add it back to the loop to be broken down again!
                                ...
                                # Split up remaining unseen percentages perfectly across countries
