...
with tab1:
    if not df.empty:
        ...
        with c_pie:
            pie_df = chart_df.groupby("Sector", as_index=False)[
                "Value (CAD)"].sum()
        ...
        with c_tree:
            chart_df["Portfolio"] = "My Portfolio"
            fig_tree = px.treemap(
                chart_df,
        ...
        # ADDED "Currency" and "Value (USD)" into the display columns list
