...
def get_correlation_matrix(hist_prices):
    """Calculates the correlation of daily returns for all assets."""
    if not hist_prices:
        return pd.DataFrame()

    returns_df = pd.DataFrame({ticker: prices.pct_change()
                              for ticker, prices in hist_prices.items()})
...
def get_portfolio_news(tickers_list):
    news_items = []
    # 1. Try to get specific ticker news
    ...
    # 2. FALLBACK: If no news was found, fetch general market news (using SPY as a proxy)
    if not news_items:
        try:
            tk = yf.Ticker("SPY")
            news = tk.news
            for n in news[:5]:  # Grab the top 5 general market articles
                pub_time = datetime.datetime.fromtimestamp(
                    n['providerPublishTime'])
                news_items.append({
                    "Ticker": "MARKET",
...
# --- MACROECONOMIC DATA ENGINE USING OFFICIAL API & SECRETS ---
...
tab1, tab_news, tab4, tab5, tab2, tab3 = st.tabs(
    ["📈 Command Center", "📰 Live News", "🔮 Forecast", "🌍 Macro", "➕ Add Asset", "⚙️ Manage Data"])
...
with tab1:
    if not df.empty:
        ...
        if not corr_matrix.empty:
            ...
            st.warning(
                "Not enough historical data to generate correlation matrix.")
...
with tab_news:
    ...
    if not df.empty and total_val_cad > 0:
        ...
        with st.spinner("Fetching latest headlines from Yahoo Finance..."):
            news_feed = get_portfolio_news(top_tickers)

        if news_feed:
            # Check if the first item is our "MARKET" fallback tag
            if news_feed[0]["Ticker"] == "MARKET":
                st.warning(
                    "No specific news found for your top assets today. Showing top overall market headlines instead.")
            else:
                st.write(
            ...
            for item in news_feed:
                # Color code the ticker tag differently if it's general market news
                ticker_display = f"🌎 {item['Ticker']}" if item['Ticker'] == "MARKET" else f"📌 {item['Ticker']}"

                st.markdown(
                    f"**[{ticker_display}]** [{item['Title']}]({item['Link']})")
                st.caption(
                    f"🗞️ {item['Publisher']} • 🕒 {item['Time'].strftime('%b %d, %Y - %I:%M %p')}")
                st.divider()
        else:
            st.info("No news found at all. Yahoo Finance might be temporarily down.")
    else:
