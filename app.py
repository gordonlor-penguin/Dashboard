...
def get_direct_etf_holdings(ticker):
    ...
    # 1. HARDCODED SKELETONS FOR FUND OF FUNDS
    # If Vanguard Canada blocks us, we bypass it completely using the target blueprints
    static_etf_maps = {
    ...
    # 2. US PROXY MAPPING
    # Translate Canadian ETFs to their US Master equivalents to bypass TSX data blocks
    ...
    if search_ticker in vanguard_urls:
        try:
            # Note: We must spoof a better header for Vanguard Institutional
            v_headers = {
    ...
    if not df.empty:
        ...
        with st.expander("🌍 True Geographic & Top 10 Holdings Exposure", expanded=False):
            with st.spinner("Executing Recursive Look-Through of Nested ETFs..."):
                ...
                # Plot Map and Pie Chart side-by-side
                ...
                with c_map:
                    fig_map = px.choropleth(
                        ...
                        title="Entire Portfolio Geographic Exposure (Including Nested ETFs & Direct Stocks)"
                    )
                ...
                if stock_exposure:
                    ...
                    with c_tree:
                        fig_stock = px.treemap(
                            ...
                            template="plotly_dark", title="Deep Look-Through Map",
