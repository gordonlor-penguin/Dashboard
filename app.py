...
def load_data():
    ...
    if not df.empty and "Currency" in df.columns and "Ticker Symbol" in df.columns:
        # Ignore CASH positions when applying the .TO suffix
...
def get_correlation_matrix(hist_prices):
    """Calculates the correlation of daily returns for all non-cash assets."""
...
def find_top_10_optimal_portfolio(price_data, risk_free_rate=0.04):
    ...
    # Calculate Sharpe for all universe stocks
    ...
    # Build dataframe with Individual Sharpe Ratio included
...
with tab1:
    if not df.empty:
        ...
        with c_chart:
            if len(port_slice) > 0:
                # Plot Actual Dollar Value Setup
                ...
                for b_tick in custom_bench_tickers:
                    if b_tick in benchmarks:
                        ...
                        if not b_slice.empty and len(b_slice) > 0:
                            # Normalize benchmarks to match the portfolio's starting dollar value for an accurate overlay
                            chart_data[b_tick] = (
                ...
                fig_line = px.line(
                    chart_data, x="Date", y="Value ($ CAD)", color="Asset", template="plotly_dark", title="Historical Value Overlay ($ CAD)")
    ...
    if st.button("Run Market Optimization", type="primary"):
        with st.spinner("Downloading 550+ stocks and running 10,000+ simulations (this may take up to 60 seconds)..."):
            ...
            if not price_history.empty:
                ...
                # Updated DataFrame with the Individual Sharpe Ratio formatting
