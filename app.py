...
def load_data():
    ...
    # Auto-fix Canadian Tickers (Ignoring Cash)
    if not df.empty:
...
def find_top_10_optimal_portfolio(price_data, risk_free_rate=0.04):
    ...
    final_sharpe = ((final_ret / 100) - risk_free_rate) / (final_vol / 100)

    results_df = pd.DataFrame({"Ticker": top_10_tickers, "Optimal Weight (%)": optimal_weights}).sort_values(
        by="Optimal Weight (%)", ascending=False)
...
with tab1:
    if not df.empty:
        ...
        c2.metric("Total Gain/Loss",
                  f"{total_ret_pct:+.2f}%", f"${total_val - total_cost:,.2f}")
        ...
        st.divider()
        st.markdown("### 🧩 Allocation & Correlation")
        ca, cb = st.columns(2)
        with ca:
        ...
        with cb:
            valid_hist = {t: p for t, p in hist_prices.items()
                          if not str(t).startswith("CASH")}
            if len(valid_hist) > 1:
                corr = pd.DataFrame({t: p.pct_change()
                                    for t, p in valid_hist.items()}).corr()
                st.plotly_chart(px.imshow(corr, text_auto=".2f", color_continuous_scale='RdBu_r',
                                template="plotly_dark", title="Asset Correlation"), use_container_width=True)
            else:
                st.info("Add more than one non-cash asset to see correlation.")

        st.divider()
        st.dataframe(grouped.style.format({"Live Price": "${:.2f}", "Value (CAD)": "${:,.2f}",
                     "Weight (%)": "{:.1f}%", "Return (%)": "{:+.1f}%"}), use_container_width=True, hide_index=True)
    else:
...
with tab_news:
    ...
    if not df.empty:
        ...
        for n in news:
            ...
            st.caption(f"{n['Publisher']} • {n['Time'].strftime('%b %d, %Y')}")
            st.divider()
    ...
    if st.button("Run Market Optimization", type="primary"):
        with st.spinner("Downloading 550+ stocks and running 10,000+ simulations (this may take up to 60 seconds)..."):
            ...
            if not price_history.empty:
                ...
                c3.metric("Expected Sharpe Ratio", f"{exp_sharpe:.2f}")
