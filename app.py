...
def load_data():
    ...
    # Safely converts strings like "0.09%" into a mathematical float
    df["MER (%)"] = pd.to_numeric(df["MER (%)"].astype(str).str.replace(
...
def get_smart_metadata(ticker_str):
    if str(ticker_str).upper().startswith("CASH"):
        ...
        # MER removed from auto-fetching
    ...
    try:
        ...
        # MER is completely removed from the dictionary returned so it never overwrites the CSV
...
if not df.empty:
    ...
    with st.spinner("Syncing Live Engines (Accelerated)..."):
        ...
        for idx, row in df.iterrows():
            ...
            needs_update = False
            # ONLY trigger metadata sync for Sector and Country now
...
if page == "📈 Command Center":
    if not df.empty:
        ...
        exploded_df = pd.DataFrame(exploded_rows, columns=[
                                   "Ticker Symbol", "Sector", "FMV (CAD)", "Asset Return (%)", "Portfolio"])
        ...
        if not exploded_df.empty:
            ...
            with c_tree:
                fig_tree = px.treemap(
                    ...
                    color="Asset Return (%)", color_continuous_scale=[
                        '#FF4B4B', '#262730', '#00C9B1'],
            ...
            st.info(
                "📊 Add assets with a quantity greater than 0 to view your Allocation charts.")
        ...
        with st.expander("🌍 True Geographic & Top 10 Holdings Exposure", expanded=False):
            with st.spinner("Executing Recursive Look-Through of Nested ETFs..."):
                ...
                geo_df = pd.DataFrame(exploded_geo_rows, columns=[
                                      "Country", "FMV (CAD)"])
                if not geo_df.empty:
                    ...
                    with c_map:
                        ...
                        fig_map.update_layout(geo=dict(
                            showframe=False, showcoastlines=False, projection_type='equirectangular'), margin=dict(l=0, r=0, t=40, b=0))
                    ...
                    st.info(
                        "🌍 Add assets with a quantity greater than 0 to view Geographic Exposure.")
                ...
                if stock_exposure:
                    ...
                    with c_tree:
                        fig_stock = px.treemap(
                            ...
                            color="Type", color_discrete_sequence=[
                                '#00C9B1', '#3b82f6', '#FF4B4B']
                        )
    ...
    if not df.empty and total_val_cad > 0:
        ...
        st.plotly_chart(fig_reb.update_layout(barmode='group', template="plotly_dark", title="Current vs Target Allocations",
                        xaxis_title="", yaxis_title="Weight (%)"), use_container_width=True)

        st.divider()
        st.subheader("🧠 Current Portfolio Optimizer (Monte Carlo)")
        st.markdown(
            "This engine simulates **10,000 random weight allocations** for your *current* assets to mathematically find the optimal mix that historically maximizes the Sharpe Ratio.")
        ...
        if st.button("Run Optimal Allocation Simulation", type="primary"):
            with st.spinner("Running Monte Carlo simulations on your assets..."):
                opt_tickers = [t for t in reb_df["Ticker Symbol"]
                               if not str(t).upper().startswith("CASH")]
                if len(opt_tickers) > 1:
                    valid_hist_data = {t: hist_prices[t] for t in opt_tickers if t in hist_prices and len(
                        hist_prices[t]) > 200}
                    if len(valid_hist_data) > 1:
                        ...
                        all_weights = np.random.random(
                            (num_ports, num_assets))
                        all_weights = all_weights / \
                            all_weights.sum(axis=1, keepdims=True)
                        ...
                        for i in range(num_ports):
                            port_vols[i] = np.sqrt(
                                np.dot(all_weights[i].T, np.dot(cov_matrix, all_weights[i])))
                        ...
                        comp_df = reb_df[["Ticker Symbol", "Name", "Portfolio Weight (%)", "Target Weight (%)"]].merge(
                            opt_result_df, on="Ticker Symbol", how="left").fillna(0)

                        st.success(
                            f"**Optimization Complete!** Max Sharpe Ratio: **{sharpe_ratios[max_sr_idx]:.2f}** | Est. Annual Return: **{port_returns[max_sr_idx]*100:.2f}%** | Volatility: **{port_vols[max_sr_idx]*100:.2f}%**")
                        ...
                        with c2:
                            ...
                            fig_opt.add_trace(go.Bar(
                                x=comp_df["Ticker Symbol"], y=comp_df["Portfolio Weight (%)"], name="Current Weight (%)", marker_color="#3b82f6"))
                            fig_opt.add_trace(go.Bar(
                                x=comp_df["Ticker Symbol"], y=comp_df["Optimal Weight (%)"], name="AI Optimal Weight (%)", marker_color="#00C9B1"))
                            st.plotly_chart(fig_opt.update_layout(barmode='group', template="plotly_dark", title="Your Current vs AI Optimal Allocations",
                                            xaxis_title="", yaxis_title="Weight (%)"), use_container_width=True)
                    else:
                        st.error(
                            "Not enough historical data available for your assets to run the simulation. Please try again later.")
                else:
                    st.error(
                        "You need at least 2 non-cash assets to run a meaningful portfolio optimization.")
    ...
    if not df.empty and total_val_cad > 0:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=np.arange(yrs+1), y=p[2],
                      name='Bull', line=dict(color='#00C9B1', dash='dash')))
        fig.add_trace(go.Scatter(x=np.arange(yrs+1), y=p[1],
                      name='Base', line=dict(color='#3b82f6', width=4)))
        fig.add_trace(go.Scatter(x=np.arange(yrs+1), y=p[0],
                      name='Bear', line=dict(color='#FF4B4B', dash='dash')))
    ...
    if st.button("Run Market Optimization", type="primary"):
        with st.spinner("Analyzing 550 stocks... (This takes ~60 seconds to download the daily data)"):
            ...
            if not history.empty:
                ...
                st.dataframe(opt_df.style.format({"Optimal Weight (%)": "{:.1f}%", "Individual Sharpe Ratio": "{:.2f}"}),
                             use_container_width=True, hide_index=True)
                ...
                with c_ch2:
                    ...
                    start = pd.DataFrame([[100.0]*3], columns=cum_perf.columns, index=[
                                         cum_perf.index[0]-pd.Timedelta(days=1)])
                    fig = px.line(pd.concat([start, cum_perf]).reset_index().melt(id_vars="index", var_name="Asset",
                                  value_name="Growth"), x="index", y="Growth", color="Asset", template="plotly_dark", title="2-Year Backtest ($100)")
                    st.plotly_chart(fig.update_traces(line=dict(
                        width=3), selector=dict(name="AI Portfolio")).update_layout(xaxis_title=""), use_container_width=True)
    ...
    with st.expander("➕ Add New Asset to Database", expanded=True):
        with st.form("add_form", clear_on_submit=True):
            ...
            mer_in = c3.number_input(
                "MER (%)", min_value=0.0, format="%.3f")
            ...
            if st.form_submit_button("Identify & Save"):
                ...
                new_row = {
                    ...
                    "Quantity": q_in,
                    "Sector": meta["Sector"],
                    "Yield (%)": meta.get("Yield (%)", 0.0),
                    "MER (%)": mer_in,
                    "P/E": meta.get("P/E", np.nan),
                    "1Y Target": meta.get("1Y Target", np.nan),
                    "Target Weight (%)": 0.0,
                    "Country": meta.get("Country", "Unknown")
                }

                pd.concat([load_data(), pd.DataFrame([new_row])],
                          ignore_index=True).to_csv(DATA_FILE, index=False)
