...
# --- NEW: MACROECONOMIC DATA ENGINE WITH USER-AGENT FIX ---


@st.cache_data(ttl=86400)
def get_macro_data():
    try:
        # FRED blocks basic python requests. We use a User-Agent header to fake a web browser.
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}

        rates_url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=FEDFUNDS"
        req_rates = urllib.request.Request(rates_url, headers=headers)
        with urllib.request.urlopen(req_rates) as response:
            rates = pd.read_csv(io.StringIO(response.read().decode(
                'utf-8')), parse_dates=['DATE'], index_col='DATE')

        cpi_url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=CPIAUCSL"
        req_cpi = urllib.request.Request(cpi_url, headers=headers)
        with urllib.request.urlopen(req_cpi) as response:
            cpi = pd.read_csv(io.StringIO(response.read().decode(
                'utf-8')), parse_dates=['DATE'], index_col='DATE')
        ...
        cpi['YoY Inflation (%)'] = pd.to_numeric(
            cpi['YoY Inflation (%)'], errors='coerce')

        return rates.dropna(), cpi.dropna()
...
tab1, tab4, tab5, tab2, tab3 = st.tabs(
    ["📈 Command Center", "🔮 Forecast", "🌍 Macro", "➕ Add Asset", "⚙️ Manage Data"])
...
if not df.empty:
    ...
    with st.spinner("Syncing live market data and fundamentals..."):
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(current_sector) or current_sector == "Unknown" or pd.isna(row.get("P/E")):
    ...
    grouped = grouped.rename(columns={
                             "Value_CAD": "Value (CAD)", "Value_USD": "Value (USD)", "Target_Weight": "Target Weight (%)"}).fillna(np.nan)
...
with tab1:
    if not df.empty:
        ...
        col1.metric("Total Value (CAD)", f"${total_val_cad:,.2f}")
        ...
        with c_bench:
            # FIXED: SPY and QQQ are permanent. The text box is just for extras.
            ...
            custom_bench_tickers = list(dict.fromkeys(
                custom_bench_tickers))  # Remove duplicates
        ...
        st.dataframe(grouped[display_cols].style.format({
            "Avg Book Price": "${:,.2f}", "Live Price": "${:,.2f}", "Value (CAD)": "${:,.2f}", "Quantity": "{:,.4g}", "Portfolio Weight (%)": "{:.2f}%", "Yield (%)": "{:.2f}%", "P/E": "{:.1f}", "Asset Return (%)": "{:+.2f}%"
...
with tab5:
    ...
    st.write("Understand the broader economic conditions driving market returns. Data sourced directly from the Federal Reserve Economic Data (FRED).")
    ...
    if not rates_df.empty and not cpi_df.empty:
        ...
        st.error(
            "Could not fetch macroeconomic data. Check your internet connection or try again later.")
