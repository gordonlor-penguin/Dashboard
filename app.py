...
def load_data():
    ...
    # Ensure all new columns exist for legacy databases
    for col in ["Yield (%)", "P/E", "Fwd P/E", "P/B", "1Y Target", "Target Weight (%)"]:
...
def get_smart_metadata(ticker_str):
    try:
        ...
        return {"Name": name, "Sector": sector, "Yield (%)": div_yield, "P/E": pe, "Fwd P/E": fwd_pe, "P/B": pb, "1Y Target": target}
    except:
        return {"Name": "Unknown", "Sector": "Unknown", "Yield (%)": 0.0, "P/E": np.nan, "Fwd P/E": np.nan, "P/B": np.nan, "1Y Target": np.nan}
...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] { padding-top: 10px; padding-bottom: 10px; font-size: 1.1rem; }
    </style>
""", unsafe_allow_html=True)

st.title("📊 Portfolio Intelligence")

# CLEANER UI: Consolidated into 4 distinct workflows
t_command, t_fundamentals, t_rebalance, t_manage = st.tabs([
    "📈 Command Center", "🧠 Fundamentals", "⚖️ Rebalance Engine", "⚙️ Database & Actions"
])
...
if not df.empty:
    ...
    with st.spinner("Syncing live market data, fundamentals, and risk metrics..."):
        needs_save = False
        for idx, row in df.iterrows():
            # Force update if critical data is missing (handles the transition to Phase 3)
            needs_update = pd.isna(
                row.get("P/E")) or pd.isna(row.get("Sector")) or row.get("Sector") == "Unknown"
            if needs_update:
    ...
    grouped = df.groupby(["Ticker Symbol", "Name", "Currency", "Sector", "Live Price", "Yield (%)", "P/E", "Fwd P/E", "P/B", "1Y Target"]).agg(
        Quantity=('Quantity', 'sum'),
        Total_Native_Cost=('Native_Cost_Total', 'sum'),
        Total_Cost_CAD=('Cost Basis (CAD)', 'sum'),
        Value_CAD=('Value (CAD)', 'sum'),
        # Takes the user's set target weight
...
with t_command:
    if not df.empty:
        ...
        st.info("Your portfolio is empty. Head to the 'Database & Actions' tab.")

# --- TAB 2: FUNDAMENTALS ---
with t_fundamentals:
    st.markdown("### 🧠 Fundamental Analysis")
    st.write("Track the underlying business health and Wall Street projections for your specific holdings.")

    if not df.empty:
        fund_cols = ["Ticker Symbol", "Name", "Sector", "Live Price",
                     "Yield (%)", "P/E", "Fwd P/E", "P/B", "1Y Target"]
        fund_df = grouped[fund_cols].copy()

        fund_df["Upside to Target (%)"] = np.where(fund_df["1Y Target"] > 0, ((
            fund_df["1Y Target"] - fund_df["Live Price"]) / fund_df["Live Price"]) * 100, np.nan)

        st.dataframe(fund_df.style.format({
            "Live Price": "${:,.2f}", "Yield (%)": "{:.2f}%", "P/E": "{:.1f}", "Fwd P/E": "{:.1f}", "P/B": "{:.1f}",
            "1Y Target": "${:,.2f}", "Upside to Target (%)": "{:+.2f}%"
        }).map(lambda x: 'color: #00C9B1' if pd.notna(x) and x > 0 else ('color: #FF4B4B' if pd.notna(x) and x < 0 else ''), subset=["Upside to Target (%)"]),
            use_container_width=True, hide_index=True)
    else:
        st.info("Add assets to view fundamentals.")

# --- TAB 3: REBALANCE ENGINE ---
with t_rebalance:
    st.markdown("### ⚖️ Automated Rebalancing")
    st.write("Set your ideal portfolio target weights below. The engine will calculate exactly how many shares you need to buy or sell *today* to align your portfolio.")
    ...
    if not df.empty:
        ...
        # Make target weight editable directly in the UI
        st.markdown(
        ...
        # Adjust for FX if USD asset
        currency_map = dict(zip(grouped["Ticker Symbol"], grouped["Currency"]))
        ...
        actions_df = edited_rebal[["Ticker Symbol", "Current Weight",
                                   "Target Weight (%)", "Value Difference", "Shares to Action"]].copy()
        actions_df.columns = [
        ...
        if st.button("💾 Save Target Weights to Database"):
            # Update the main CSV with the new target weights
            target_map = dict(
        ...
        st.info("Add assets to use the rebalancing engine.")

# --- TAB 4: MANAGE DATA & ADD ASSETS ---
with t_manage:
            ...
            if st.form_submit_button("Identify & Save"):
                ...
                with st.spinner(f"Analyzing {ticker}..."):
                    ...
                    new_row = pd.DataFrame([{
                        ...
                        "P/E": meta["P/E"], "Fwd P/E": meta["Fwd P/E"], "P/B": meta["P/B"],
                        "1Y Target": meta["1Y Target"], "Target Weight (%)": 0.0
                    }])
    ...
    st.markdown("#### 📝 Edit Ledger")
    st.caption(
        "Directly modify your raw transaction data. The app combines identical tickers automatically.")
