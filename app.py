...
# ----- THE BULLETPROOF METADATA ENGINE -----
...
def get_smart_metadata(ticker_str):
    ...
    try:
        ...
        # Guard against completely empty Yahoo responses
        if not info:
            return {"Name": str(ticker_str), "Sector": "Unknown", "Yield (%)": 0.0, "P/E": np.nan, "1Y Target": np.nan}

        name = info.get('longName') or info.get('shortName') or str(ticker_str)

        # 1. Yield Extraction (Crash-proof)
        ...
        if raw_yield is not None:
            try:
                if isinstance(raw_yield, str) and '%' in raw_yield:
                    ...
                    if 0.0 < div_yield < 0.30:  # Fix decimals
                        div_yield *= 100
            except (ValueError, TypeError):
                div_yield = 0.0

        # 2. P/E Extraction & Calculation (Crash-proof)
        ...
        if pe is None or str(pe).strip() in ["", "nan", "None", "N/A"]:
            ...
            try:
                pe = float(pe)
                if pe <= 0:  # Force Net Loss companies to N/A
                    pe = np.nan
            except (ValueError, TypeError):
                pe = np.nan

        # 3. Target Price (Crash-proof)
        try:
            target = float(info.get('targetMeanPrice', np.nan))
        except (ValueError, TypeError):
            target = np.nan

        # 4. Sector Extraction (Crash-proof)
...
if not df.empty:
    ...
    with st.spinner("Syncing live market data and fundamentals..."):
        ...
        for idx, row in df.iterrows():
            ...
            # Smart trigger to fix previously corrupted P/E or Missing Sectors
            needs_update = False
            if pd.isna(current_sector) or current_sector in ["Unknown", "nan", "NaN"]:
                needs_update = True
            elif pd.isna(row.get("P/E")) and str(row.get("Ticker Symbol")).upper() != "CASH":
                needs_update = True

            if needs_update:
                meta = get_smart_metadata(row["Ticker Symbol"])
                for k, v in meta.items():
                    # Preserve existing valid yields to avoid overwriting them with 0.0s
    ...
    # Safely group elements allowing NaN for P/E and Yield
