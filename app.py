...
def get_finnhub_insights(tickers_list):
    ...
    for t in tickers_list:
        if str(t).upper().startswith("CASH"):
            continue
        fh_ticker = str(t).replace(".TO", "")  # Strip .TO for API
        insights[t] = {}
        ...
        try:
            ...
            if res_soc and 'reddit' in res_soc and len(res_soc['reddit']) > 0:
                # Grab the most recent day's chatter
...
def get_news():
    ...
    # Grab Top 10 General Market/Economy Headlines
    try:
...
if page == "📈 Command Center":
    if not df.empty:
        ...
        with c_map:
            # Map generation based on Country field
            map_df = grouped.groupby("Country", as_index=False)[
                "Value (CAD)"].sum()
        ...
        with c_pie:
            st.plotly_chart(px.pie(grouped, names="Sector", values="Value (CAD)", template="plotly_dark", title="Sector Breakdown", hole=0.4).update_traces(
                textposition='inside', textinfo='percent+label').update_layout(showlegend=False, margin=dict(t=40, b=0)), use_container_width=True)
