...
# --- HYBRID ETF SCRAPING ENGINE (INVESCO, iSHARES, VANGUARD) ---
...
def get_direct_etf_holdings(ticker):
    ...
    # 1. INVESCO (QQQ and QQC.TO)
    if ticker in ["QQQ", "QQC.TO"]:
        try:
            # QQC tracks the exact same Nasdaq-100 index as QQQ, so we map them to the same master file
    ...
    # 2. ISHARES (Add your specific iShares URLs here)
    ishares_urls = {
        ...
        "XAW.TO": ""  # Replace with URL if you own it
    }
    if ticker in ishares_urls and ishares_urls[ticker] != "":
        try:
            res = requests.get(
                ishares_urls[ticker], headers=headers, timeout=5)
            df = pd.read_csv(io.StringIO(res.text), skiprows=9)
            df = df.dropna(subset=['Ticker'])
            df = df[df['Ticker'] != '-']
            df = df.rename(columns={"Ticker": "Asset"})
            df['Weight (%)'] = pd.to_numeric(
                df['Weight (%)'], errors='coerce') / 100
            return df[['Asset', 'Weight (%)']].dropna()
        except:
            pass

    # 3. VANGUARD (Add your specific Vanguard URLs here)
    vanguard_urls = {
        "VFV.TO": "",  # Replace with URL if you own it
        "VOO": ""
    }
    if ticker in vanguard_urls and vanguard_urls[ticker] != "":
        try:
            res = requests.get(
                vanguard_urls[ticker], headers=headers, timeout=5)
            # Adjust skiprows depending on Vanguard's CSV format
    ...
    # 4. FALLBACK: YAHOO FINANCE TOP 10
    # If the URL is blank or fails, gracefully fallback to the top 10 holdings via Yahoo Finance
    try:
        tk = yf.Ticker(ticker)
        ...
        if holdings is not None and not holdings.empty:
            ...
            # yfinance returns weights as decimals (e.g. 0.05 for 5%)
...
@st.cache_data(ttl=604800)  # Cache for a week so we don't spam Yahoo Finance
...
# --- FINNHUB ANALYST & REDDIT SENTIMENT ENGINE ---


@st.cache_data(ttl=3600)
def get_finnhub_insights(tickers_list):
    ...
    for t in tickers_list:
        if str(t).upper().startswith("CASH"):
            continue
        fh_ticker = str(t).replace(".TO", "")
        insights[t] = {}

        try:
            url_rec = f"https://finnhub.io/api/v1/stock/recommendation?symbol={fh_ticker}&token={api_key}"
            res_rec = requests.get(url_rec, timeout=3).json()
            if res_rec and isinstance(res_rec, list) and len(res_rec) > 0:
                insights[t]['rec'] = res_rec[0]
        except:
            pass

        try:
            url_soc = f"https://finnhub.io/api/v1/stock/social-sentiment?symbol={fh_ticker}&token={api_key}"
            res_soc = requests.get(url_soc, timeout=3).json()
            if res_soc and 'reddit' in res_soc and len(res_soc['reddit']) > 0:
                insights[t]['reddit'] = res_soc['reddit'][0]
        except:
            pass
...
def get_news():
    ...
    try:
        rss_url = "https://news.google.com/rss/search?q=stock+market+economy+finance&hl=en-US&gl=US&ceid=US:en"
        ...
        for item in root.findall('./channel/item')[:10]:
            ...
            news_items.append({"Ticker": "GLOBAL MARKET", "Title": item.find(
...
if page == "📈 Command Center":
    if not df.empty:
        ...
        # FINNHUB ANALYST & SOCIAL EXPANDER
        with st.expander("🎯 Wall Street Consensus & Reddit Sentiment (Finnhub)", expanded=False):
            if st.secrets.get("finnhub_api_key"):
                top_stocks = [t for t in grouped["Ticker Symbol"].tolist(
                ) if not t.startswith("CASH")][:5]
                insights = get_finnhub_insights(top_stocks)
                if insights:
                    cols = st.columns(len(insights))
                    for idx, (t, data) in enumerate(insights.items()):
                        with cols[idx]:
                            st.markdown(f"**{t}**")
                            if 'rec' in data:
                                r = data['rec']
                                score = r.get(
                                    'strongBuy', 0)*2 + r.get('buy', 0) - r.get('sell', 0) - r.get('strongSell', 0)*2
                                status = "🟢 BUY" if score > 2 else (
                                    "🔴 SELL" if score < -2 else "🟡 HOLD")
                                st.subheader(status)
                                st.caption(
                                    f"Wall St: {r.get('buy', 0)} Buy | {r.get('hold', 0)} Hold | {r.get('sell', 0)} Sell")
                            else:
                                st.write("Wall St: No Data")

                            if 'reddit' in data:
                                rd = data['reddit']
                                reddit_score = rd.get('score', 0)
                                if reddit_score > 0.5:
                                    r_stat = "🔥 Bullish"
                                    ...
                                    r_stat = "😐 Neutral"
                                st.markdown(f"*Reddit/WSB:* **{r_stat}**")
                                st.caption(
                                    f"{rd.get('mention', 0)} recent mentions")
                            else:
                                st.caption("Reddit: No chatter")
                else:
                    st.info("No ratings found.")
            else:
                st.info(
                    "Add `finnhub_api_key` to your Streamlit secrets to unlock Wall Street & Reddit Sentiment.")
        ...
        # --- NEW GEOGRAPHIC MAP EXPANDER (WITH TRUE UNDERLYING STOCK SCRAPING) ---
        with st.expander("🌍 Geographic Exposure Map (True Underlying Stock Lookup)", expanded=False):
            with st.spinner("Fetching daily issuer files and analyzing geography..."):
                stock_exposure = []
                exploded_geo_rows = []
                ...
                for _, row in grouped[grouped["Value (CAD)"] > 0].iterrows():
                    ...
                    if "ETF" in sector:
                        ...
                        if holdings_df is not None and not holdings_df.empty:
                            ...
                            # Allocate remaining unmapped value to the ETF's home country
                    ...
                    # Single stocks just go directly into the pool
                    stock_exposure.append({
                        "Asset": ticker,
                        "Value (CAD)": val,
                        "Type": "Direct Holding",
                ...
                # Fetch countries for all the underlying stocks found inside the ETFs
                ...
                fig_map = px.choropleth(
                    ...
                    title="True Geographic Exposure (Based on Actual Underlying Stocks)"
                )
                ...
                # Show ledger breakdown of countries
                st.dataframe(
                ...
                # Show deep dive of the underlying stocks
                if stock_exposure:
    ...
    st.header("📰 Top 10 Global Market Headlines")
    with st.spinner("Fetching global economy & stock market news..."):
        news = get_news()
        if news:
            for n in news:
                st.markdown(
