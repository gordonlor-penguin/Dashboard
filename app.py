...
def get_smart_metadata(ticker_str):
    ...
    try:
        ...
        # --- BULLETPROOF MER SCRAPER (Expense Ratio Net) ---
        # Targets the exact 'Expense Ratio (net)' variable from Yahoo Finance
        raw_mer = info.get('expenseRatio') or info.get(
            'annualReportExpenseRatio')

        # Failsafe scan if Yahoo renames the key
        if raw_mer is None:
        ...
        if raw_mer is not None:
            try:
                ...
                if parsed_mer < 0.1:  # Normalizes outputs like 0.0009 back into 0.09%
                    mer = parsed_mer * 100
                else:
                    mer = parsed_mer
            except:
