...
# --- NEW: MACROECONOMIC DATA ENGINE (DIRECT API) ---


@st.cache_data(ttl=86400)  # Cache for 24 hours
def get_macro_data():
    try:
        # Pull directly from FRED's public CSV endpoints (no extra library needed)
        rates_url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=FEDFUNDS"
        cpi_url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=CPIAUCSL"

        rates = pd.read_csv(rates_url, parse_dates=['DATE'], index_col='DATE')
        cpi = pd.read_csv(cpi_url, parse_dates=['DATE'], index_col='DATE')

        # Calculate Year-Over-Year Inflation on the full dataset
        cpi['YoY Inflation (%)'] = cpi['CPIAUCSL'].pct_change(12) * 100

        # Filter down to the last 5 years to match your dashboard
        start_date = pd.Timestamp.now() - pd.DateOffset(years=5)
        rates = rates[rates.index >= start_date]
        cpi = cpi[cpi.index >= start_date]

        # Convert index to datetime and remove timezone if any
        rates.index = rates.index.tz_localize(None)
        cpi.index = cpi.index.tz_localize(None)

        # Handle cases where columns might be read as object/strings due to '.' for missing values
    ...
    if not df.empty:
        ...
        with c_bench:
            bench_input = st.text_input(
                "📈 Add Benchmarks (Comma-separated tickers)", value="SPY, QQQ")
            custom_bench_tickers = [x.strip().upper()
                                    for x in bench_input.split(",") if x.strip()]
        ...
        with c_risk:
            ...
            primary_bench = custom_bench_tickers[0] if custom_bench_tickers else None
