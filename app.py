...
def get_direct_etf_holdings(ticker):
    ...
    if search_ticker in vanguard_urls:
        # CHANGE 1: Vanguard extraction update
        try:
            v_headers = {
            ...
            # Look for Name and Country columns
            name_col = next(
                (col for col in df.columns if 'Description' in col or 'Name' in col), None)
            market_col = next(
                (col for col in df.columns if 'Market' in col or 'Country' in col), None)

            if weight_col:
                # Prevent foreign stocks from being dropped by filling blank tickers with their company name
                if ticker_col and name_col:
                    df[ticker_col] = df[ticker_col].fillna(df[name_col])
                elif not ticker_col and name_col:
                    ticker_col = name_col
                ...
                # If Vanguard provides the country, return it!
                if market_col:
                    df['Country'] = df[market_col]
                    return df[['Asset', 'Weight (%)', 'Country']].dropna(subset=['Weight (%)'])
                else:
                    return df[['Asset', 'Weight (%)']].dropna()
        except:
            pass
...
if not df.empty:
    ...
    # Fast Vectorized Math - Explicity computing FMV (Fair Market Value)
...
if page == "📈 Command Center":
    if not df.empty:
        ...
        display_cols = ["Ticker Symbol", "Name", "Sector", "Country", "Quantity", "Avg Book Price",
                        "Live Price", "FMV (CAD)", "Portfolio Weight (%)", "Yield (%)", "P/E", "Asset Return (%)"]
        ...
        # --- RECURSIVE GEOGRAPHIC MAP & TOP 10 HOLDINGS ---
        def get_etf_remainder_country(ticker, base):
            t = ticker.upper()
            if t in ["VUN.TO", "VTI", "VOO", "VFV.TO", "QQQ", "QQC", "QQC.TO", "ITOT", "IVV", "SPY"]:
                return "United States"
            if t in ["VCN.TO", "XIC.TO", "VDY.TO", "XIU.TO"]:
                return "Canada"
            # Removed the generic Developed/Emerging groups so the script defaults to the underlying stocks
            return base
        ...
        with st.expander("🌍 True Geographic & Top 10 Holdings Exposure", expanded=False):
            with st.spinner("Executing Recursive Look-Through of Nested ETFs..."):
                ...
                KNOWN_RECURSIVE_ETFS = ["VTI", "VFV.TO", "VEQT.TO", "VUN.TO", "VCN.TO",
                                        "VIU.TO", "VEE.TO", "VOO", "QQQ", "QQC", "QQC.TO", "XEQT.TO", "ITOT", "IVV"]
                        ...
                        while queue:
                            ...
                            if holdings_df is not None and not holdings_df.empty:
                                ...
                                for _, h_row in holdings_df.iterrows():
                                    ...
                                    # CHANGE 2: Safely extract the country if we found it
                                    h_country = h_row["Country"] if "Country" in holdings_df.columns else None
                ...
                for item in stock_exposure:
                    if item["Type"] == "Hidden ETF Holding":
                        # CHANGE 3: Check if Vanguard explicitly told us the country
                        if item.get("Direct_Country") and pd.notna(item["Direct_Country"]):
                            country = item["Direct_Country"]
                        # Otherwise fallback to the Yahoo Finance bulk search
                        else:
                            country = countries_map.get(
                                item["Asset"], "United States")

                        if country in ["U.S.", "United States of America"]:
                            country = "United States"
                        if country == "U.K.":
                            country = "United Kingdom"
